  ...
  const connectMetamask = useCallback(async () => {
    ...
    try {
      ...
      const targetProvider =
        ethereum?.providers?.find((p) => p.isMetaMask) || ethereum || null;
      if (!targetProvider) throw new Error("MetaMask not found");
      await ensureCeloNetwork(targetProvider);
      await targetProvider.request({ method: "eth_requestAccounts" });
      await attachProvider(targetProvider);
    } finally {
